...
//    #[cfg(feature="optix")]
    unsafe { return optix_render_rgb(w, h, spp, out); }
...
fn idx(x: i32, y: i32, w: i32) -> usize { (y as usize) * (w as usize) + (x as usize) }
#[inline] fn clamp(v: i32, lo: i32, hi: i32) -> i32 { v.max(lo).min(hi) }
...
fn cfa_at(x: i32, y: i32, pattern: i32) -> CFA {
    match pattern {
        0 => if (y & 1) == 0 { if (x & 1) == 0 { CFA::R } else { CFA::G } } else { if (x & 1) == 0 { CFA::G } else { CFA::B } },
        1 => if (y & 1) == 0 { if (x & 1) == 0 { CFA::B } else { CFA::G } } else { if (x & 1) == 0 { CFA::G } else { CFA::R } },
        2 => if (y & 1) == 0 { if (x & 1) == 0 { CFA::G } else { CFA::R } } else { if (x & 1) == 0 { CFA::B } else { CFA::G } },
        _ => if (y & 1) == 0 { if (x & 1) == 0 { CFA::G } else { CFA::B } } else { if (x & 1) == 0 { CFA::R } else { CFA::G } },
    }
}
...
fn demosaic_amaze(raw16: &[u16], w: i32, h: i32, pattern: i32) -> Vec<f32> {
    ...
    for y in 0..h { for x in 0..w {
        ...
        if c == CFA::R || c == CFA::B {
            ...
            let grad_h = (g_l1 - g_r1).abs() + (2.0*c0 - c_l2 - c_r2).abs() * 0.5 + (c_l1 - c_r1).abs();
            let grad_v = (g_u1 - g_d1).abs() + (2.0*c0 - c_u2 - c_d2).abs() * 0.5 + (c_u1 - c_d1).abs();
    ...
    for y in 0..h { for x in 0..w {
        if matches!(cfa_at(x, y, pattern), CFA::G) {
            ...
            let has_red_h  = matches!(cfa_at(x-1,y,pattern), CFA::R) || matches!(cfa_at(x+1,y,pattern), CFA::R);
            let has_red_v  = matches!(cfa_at(x,y-1,pattern), CFA::R) || matches!(cfa_at(x,y+1,pattern), CFA::R);
            let has_blue_h = matches!(cfa_at(x-1,y,pattern), CFA::B) || matches!(cfa_at(x+1,y,pattern), CFA::B);
            let has_blue_v = matches!(cfa_at(x,y-1,pattern), CFA::B) || matches!(cfa_at(x,y+1,pattern), CFA::B);
            let r_est = if has_red_h {
                0.5*(getf(&r,w,h,x-1,y)+getf(&r,w,h,x+1,y)) - 0.5*(getf(&g,w,h,x-1,y)+getf(&g,w,h,x+1,y)) + g0
            } else if has_red_v {
                0.5*(getf(&r,w,h,x,y-1)+getf(&r,w,h,x,y+1)) - 0.5*(getf(&g,w,h,x,y-1)+getf(&g,w,h,x,y+1)) + g0
            } else { g0 };
            let b_est = if has_blue_h {
                0.5*(getf(&b,w,h,x-1,y)+getf(&b,w,h,x+1,y)) - 0.5*(getf(&g,w,h,x-1,y)+getf(&g,w,h,x+1,y)) + g0
            } else if has_blue_v {
                0.5*(getf(&b,w,h,x,y-1)+getf(&b,w,h,x,y+1)) - 0.5*(getf(&g,w,h,x,y-1)+getf(&g,w,h,x,y+1)) + g0
            } else { g0 };
    ...
    for y in 0..h { for x in 0..w {
        match cfa_at(x, y, pattern) {
            CFA::R => {
                let g0 = getf(&g, w, h, x, y);
                let b_est = 0.25*((getf(&b,w,h,x-1,y-1)-getf(&g,w,h,x-1,y-1))
                    +(getf(&b,w,h,x+1,y-1)-getf(&g,w,h,x+1,y-1))
                    +(getf(&b,w,h,x-1,y+1)-getf(&g,w,h,x-1,y+1))
                    +(getf(&b,w,h,x+1,y+1)-getf(&g,w,h,x+1,y+1))) + g0;
                setf(&mut b, w, h, x, y, b_est);
            }
            CFA::B => {
                let g0 = getf(&g, w, h, x, y);
                let r_est = 0.25*((getf(&r,w,h,x-1,y-1)-getf(&g,w,h,x-1,y-1))
                    +(getf(&r,w,h,x+1,y-1)-getf(&g,w,h,x+1,y-1))
                    +(getf(&r,w,h,x-1,y+1)-getf(&g,w,h,x-1,y+1))
                    +(getf(&r,w,h,x+1,y+1)-getf(&g,w,h,x+1,y+1))) + g0;
                setf(&mut r, w, h, x, y, r_est);
            }
            _ => {}
        }
    ...
    for i in 0..n { rg[i] = r[i] - g[i]; bg[i] = b[i] - g[i]; }
...
fn percentile(arr: &[f32], p: f32) -> f32 {
    ...
    let _ = v.select_nth_unstable_by(k, |a,b| a.partial_cmp(b).unwrap());
    v[k].max(1e-6)
}
...
fn aces_film(x: f32) -> f32 { let a=2.51; let b=0.03; let c=2.43; let d=0.59; let e=0.14; ((x*(a*x+b))/(x*(c*x+d)+e)).clamp(0.0,1.0) }
fn tonemap_aces(v: [f32;3]) -> [f32;3] { [aces_film(v[0]), aces_film(v[1]), aces_film(v[2])] }
fn oetf_srgb(x: f32) -> f32 { if x <= 0.0031308 { 12.92*x } else { 1.055*x.powf(1.0/2.4)-0.055 } }
...
fn save_png_srgb_from_acescg(path: &str, w: i32, h: i32, img_aces: &[f32], exposure: f32) {
    ...
    for y in 0..h { for x in 0..w {
        let i = idx(x,y,w)*3;
        let a = [img_aces[i+0]*exposure, img_aces[i+1]*exposure, img_aces[i+2]*exposure];
...
fn pq_oetf_from_nits(nits: f32) -> f32 {
    ...
    let l_m1 = l.powf(m1);
    ((c1 + c2*l_m1) / (1.0 + c3*l_m1)).powf(m2).clamp(0.0,1.0)
}
...
fn main() {
    ...
    for i in 0..(w*h) as usize { let r=rgb[i*3];let g=rgb[i*3+1];let b=rgb[i*3+2]; lums.push(0.2126*r+0.7152*g+0.0722*b); }
    ...
    save_avif_rec2100_pq_from_acescg("pt_bayer_pq.avif", w, h, &demosaiced, exp_raw, 1000.0);
